...
-- table-1
create table users(
user_id int, created_at varchar(100), company_id int, language varchar(50), activated_at varchar(100), state varchar(50));
...
update users set temp_created_at = str_to_date(created_at, '%Y-%m-%d %H:%i');
alter table users drop column created_at;
alter table users change column temp_created_at created_at datetime;

alter table users add column temp_activated_at datetime;
update users set temp_activated_at = str_to_date(activated_at, '%Y-%m-%d %H:%i');
...
user_id int, occurred_at varchar(100), action varchar(100), user_type int 
);
